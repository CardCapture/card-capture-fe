...
const NAV_ITEMS = [
  { key: 'account', label: 'Account Settings', icon: <Settings className="w-5 h-5 mr-2" /> },
  { key: 'users', label: 'Manage Users', icon: <span className="mr-2">👥</span> },
  { key: 'subscription', label: 'Manage Subscription', icon: <span className="mr-2">💳</span> },
];
...
const AdminSettings: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSchool = async () => {
      if (active !== 'subscription' || !session?.user?.id) {
        console.log('Debug - Not fetching school:', { active, userId: session?.user?.id });
        return;
      }
      ...
      try {
        ...
        const apiBaseUrl = import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000';
        console.log('Debug - Fetching school from:', `${apiBaseUrl}/schools/${profile.school_id}`);
        const res = await fetch(`${apiBaseUrl}/schools/${profile.school_id}`, {
          headers: session?.access_token ? { 'Authorization': `Bearer ${session.access_token}` } : {},
        });
        console.log('Debug - School fetch response:', res.status, res.statusText);
        ...
        setSchoolError(err.message || 'Error fetching school record');
        setSchool(null);
      } finally {
  ...
  switch (active) {
    case 'account':
      ...
      content = <Card><CardContent className="p-6 text-gray-500">Account settings content coming soon.</CardContent></Card>;
      break;
    case 'users':
          ...
          <CardContent className="p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2">
              <div>
                ...
                <div className="text-muted-foreground text-sm mb-4">User Management</div>
              </div>
              <Button onClick={() => setInviteDialogOpen(true)} className="self-start sm:self-auto">Invite User</Button>
            </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="px-5 py-4 text-right">Last Login</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {loading ? (
                    ...
                    users.map((user, idx) => (
                      <TableRow
                        ...
                        className={`cursor-pointer transition hover:bg-muted ${idx % 2 === 1 ? 'bg-muted/50' : ''}`}
                      >
                        <TableCell className="px-5 py-4">{user.first_name} {user.last_name}</TableCell>
                        <TableCell className="px-5 py-4">{user.email}</TableCell>
                        <TableCell className="px-5 py-4 text-center">{user.role}</TableCell>
                        <TableCell className="px-5 py-4 text-right">{user.last_sign_in_at ? new Date(user.last_sign_in_at).toLocaleDateString(undefined, { month: 'short', day: '2-digit', year: 'numeric' }) : '-'}</TableCell>
                      </TableRow>
                    ))
                  )}
                </TableBody>
          ...
          <Card className="bg-white shadow-lg rounded-2xl max-w-2xl w-full mx-auto border border-gray-100">
            <CardContent className="p-8">
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <div className="text-2xl md:text-3xl font-bold text-gray-900">Manage Subscription</div>
                  <div className="text-gray-500 text-base md:text-lg">Control your billing plan and payment details below.</div>
                </div>
              </div>
              {/* Pricing Info Block */}
              {schoolLoading ? (
                <div className="my-10 text-gray-500 text-lg">Loading plan details...</div>
              ) : schoolError ? (
                <div className="my-10 text-red-500 text-lg">{schoolError}</div>
              ) : school ? (
                <div className="mb-10">
                  <div className="flex items-center gap-3 mb-2">
                    <span className="text-xl md:text-2xl font-semibold capitalize text-gray-800">{school.pricing_tier} Plan</span>
                    <span className="bg-blue-50 text-blue-700 font-bold px-4 py-1 rounded-full text-lg md:text-xl shadow-sm border border-blue-100">{PRICING_DISPLAY[school.pricing_tier] || 'N/A'}</span>
                  </div>
                  <div className="text-gray-600 text-base md:text-lg mb-4">
                    You're currently on the <span className="font-semibold capitalize">{school.pricing_tier}</span> plan, billed annually.
                  </div>
                  {/* Call to Action */}
                  <Button
                ...
                <div className="my-10 text-gray-500 text-lg">No school record found.</div>
              )}
            </CardContent>
          </Card>
        </div>
