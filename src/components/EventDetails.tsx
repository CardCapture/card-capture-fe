...
import { Camera, Download, Trash2, CheckCircle, FileText, FolderOpen, Loader2, X, Info, Archive, CalendarDays, CheckCircle2, PlusCircle, Upload, UserPlus, ChevronRight, ZoomIn, ZoomOut, Pencil, Check } from 'lucide-react';
...
const getSignedImageUrl = async (imagePath: string) => {
  ...
  console.log('getSignedImageUrl: Requesting signed URL for path:', JSON.stringify(imagePath));
...
const ReviewImagePanel = ({ 
  ...
  return (
    <div className="relative flex-1 flex flex-col overflow-hidden bg-white rounded-lg">
      ...
      <div 
        ref={containerRef}
        className={`flex-1 overflow-hidden ${draggingRef.current ? 'cursor-grabbing' : 'cursor-grab'}`}
...
const Dashboard = () => {
  ...
  const { cards, fetchCards, setReviewModalState } = useCardsOverride(selectedEvent?.id);
  ...
  const [selectedCardForReview, setSelectedCardForReview] = useState<ProspectCard | null>(null);
      ...
      if (eventId) {
        ...
        if (event) {
          ...
          toast({
            ...
            description: "The requested event could not be found. Redirected to the first available event.",
            variant: "destructive",
          });
        }
      } else {
  ...
  // --- Callbacks & Effects ---
  const handleFieldReview = useCallback((fieldKey: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!selectedCardForReview) return;

    // Update the local state immediately
    const updatedCard = { ...selectedCardForReview };
    const currentValue = formData[fieldKey] ?? selectedCardForReview.fields[fieldKey]?.value ?? '';
    ...
    toast({
      ...
      description: `${dataFieldsMap.get(fieldKey) || fieldKey} has been marked as reviewed.`,
      variant: "default",
    });
  }, [selectedCardForReview, formData, toast, dataFieldsMap]);

  const handleRowClick = useCallback((card: ProspectCard) => { 
  ...
  useEffect(() => {
    if (selectedCardIdRef.current && cards.length > 0) {
      const updatedCard = cards.find(card => card.id === selectedCardIdRef.current);
      if (updatedCard) {
  ...
  const columns = useMemo<ColumnDef<ProspectCard>[]>(() => [
    ...
    { accessorKey: 'createdAt', header: 'Date added', cell: ({ row }) => formatDateOrTimeAgo(row.original.createdAt), enableSorting: true },
    { accessorKey: 'status', header: 'Status', cell: ({ row }) => { 
      ...
      if (currentStatus === 'needs_human_review') {
        ...
        displayText = currentStatus ? currentStatus.charAt(0).toUpperCase() + currentStatus.slice(1).replace(/_/g, ' ') : 'Unknown';
      }
    ...
    }, enableSorting: true, sortDescFirst: true, sortingFn: (rowA, rowB) => {
    ...
    ...reviewFieldOrder.map(fieldKey => ({
      ...
      cell: ({ getValue, row }) => {
        ...
        if (fieldKey === 'date_of_birth') formattedValue = formatBirthday(value);
        let tooltipContent = reviewNotes || (needsReview ? "Needs human review" : null);
        return (
          <TooltipProvider delayDuration={100}>
            <div className="flex items-center gap-1.5">
              {showIcon && (
                <Tooltip>
                  <TooltipTrigger>
                    <div className="flex h-3 w-3 items-center justify-center rounded-full bg-red-400 flex-shrink-0 text-white text-[8px] font-bold leading-none">!</div>
                  </TooltipTrigger>
                  {tooltipContent && (
    ...
    try {
      ...
      const onUploadStart = () => {
        toast({
          ...
          description: "Your card is being processed. You can continue scanning more cards.",
          variant: "default",
          duration: 4000,
          className: "bg-white border-green-200 shadow-md rounded-lg animate-in fade-in-0 slide-in-from-top-2",
          action: (
      ...
      const data = await uploadCard(file, selectedEvent.id, selectedEvent.school_id, onUploadStart);
  ...
  const handleReviewSave = async () => {
    ...
    try {
      ...
      const allFieldsReviewed = Object.values(updatedFields).every(
        field => field.reviewed === true && field.requires_human_review === false
      );

      // Save all field updates at once
      const response = await fetch(`${apiBaseUrl}/save-review/${selectedCardForReview.id}`, {
        method: 'POST',
      ...
      toast({
        ...
        description: allFieldsReviewed ? "All fields have been reviewed and saved." : "Changes saved successfully.",
        variant: "default",
      });
  ...
  const confirmArchiveAction = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || `Failed to archive card (${response.status})`);
      }
      ...
      toast({
        ...
        description: error.message || "Failed to archive card. Please try again.",
        variant: "destructive",
      });
      setIsArchiveConfirmOpen(false);
    }
  ...
  const downloadCSV = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || `Failed to mark cards as exported (${response.status})`);
      }
      ...
      const csvContent = [
        ...
        ...selectedRows.map(row => {
          ...
          return [escapeCsvValue(eventName), ...Array.from(dataFieldsMap.keys())
            .map(key => escapeCsvValue(row.original.fields?.[key]?.value ?? ''))]
            .join(',');
        }),
      ].join('\n');
      ...
      toast({
        ...
        description: `${selectedIds.length} ${selectedIds.length === 1 ? 'card' : 'cards'} exported successfully.`,
        variant: "default",
      });
    } catch (error: any) {
  ...
  const zoomOut = useCallback(() => setZoom(z => Math.max(z / 1.2, 0.47)), []);
  ...
  useEffect(() => {
    ...
    async function updateImageUrl() {
      if (selectedCardForReview?.image_path) {
        console.log('updateImageUrl: Calling getSignedImageUrl with:', selectedCardForReview.image_path);
        const signedUrl = await getSignedImageUrl(selectedCardForReview.image_path);
        setImageUrlState(signedUrl);
        console.log('updateImageUrl: setImageUrlState to', signedUrl);
      } else {
        setImageUrlState('');
        console.log('updateImageUrl: No image_path, setImageUrlState to empty string');
      }
    }
    updateImageUrl();
  ...
  const [eventNameInput, setEventNameInput] = useState(selectedEvent?.name || '');
  ...
  const handleSaveEventName = async () => {
    ...
    try {
      ...
      toast({ title: 'Event name updated', description: 'The event name was updated successfully.' });
      setIsEditingEventName(false);
      setSelectedEvent(ev => ev ? { ...ev, name: eventNameInput.trim() } : ev);
      fetchEvents();
    } catch (error: any) {
      setEventNameError(error.message || 'Failed to update event name');
      toast({ title: 'Error', description: error.message || 'Failed to update event name', variant: 'destructive' });
    } finally {
        ...
        <Card className="mb-6">
          <CardContent className="flex flex-col md:flex-row items-start justify-between gap-4 p-6">
              ...
              <h1 className="text-2xl font-semibold text-gray-900 mb-2 text-left flex items-center gap-2">
                {isEditingEventName ? (
                  <>
                    ...
                    >
                      {eventNameLoading ? <Loader2 className="animate-spin w-5 h-5" /> : <Check className="w-5 h-5" />}
                    </button>
                    <button
            ...
            <div className="flex flex-wrap items-center justify-end gap-2 mt-2 md:mt-0">
              <Badge variant="outline" className="inline-flex items-center text-indigo-700 border-indigo-200 bg-indigo-50">
                <Info className="w-4 h-4 mr-1 text-indigo-500" />
                Need Review: {getStatusCount('needs_human_review')}
              </Badge>
              <Badge variant="outline" className="inline-flex items-center text-green-700 border-green-200 bg-green-50">
                <CheckCircle className="w-4 h-4 mr-1 text-green-500" />
                Ready for Export: {getStatusCount('reviewed')}
              </Badge>
              <Badge variant="outline" className="inline-flex items-center text-slate-700 border-slate-200 bg-slate-50">
                <Download className="w-4 h-4 mr-1 text-slate-500" />
                Exported: {getStatusCount('exported')}
              </Badge>
              <Badge variant="outline" className="inline-flex items-center text-gray-600 border-gray-200 bg-gray-50">
            ...
            <div className="flex flex-col gap-4 mb-4">
              ...
              {selectedTab === 'ready_to_export' && (
                <div className="flex items-center gap-3 text-xs text-gray-500">
                  ...
                  />
                  <Label htmlFor="hide-exported" className="text-xs text-gray-500 ml-2">
                    Hide Exported
                  </Label>
                </div>
              )}
                  ...
                  <div className="w-full flex justify-between items-center px-4 py-2.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-semibold text-blue-800">
                        {Object.keys(rowSelection).length} {Object.keys(rowSelection).length === 1 ? 'Card' : 'Cards'} Selected
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      {selectedTab === 'archived' ? (
                 ...
                 <TableHeader className="bg-gray-50 sticky top-0 z-10">
                   ...
                   {table.getRowModel().rows.length > 0 ? (
                     table.getRowModel().rows.map(row => (
                       <TableRow key={row.id} data-state={row.getIsSelected() && "selected"} className="hover:bg-gray-100 cursor-pointer" onClick={() => handleRowClick(row.original)}>
                         {row.getVisibleCells().map(cell => (
                           <TableCell key={cell.id} className="px-4 py-3 whitespace-nowrap text-sm text-gray-700">{flexRender(cell.column.columnDef.cell, cell.getContext())}</TableCell>
                         ))}
                       </TableRow>
                     ))
                   ) : (
                     <TableRow>
                       <TableCell colSpan={13} className="h-24">
                        <div className="flex flex-col items-center justify-center h-full gap-2">
                          {selectedTab === 'needs_human_review' ? (
                            <>
                              <div className="text-sm font-medium text-gray-900">Nice work! No cards need review right now.</div>
                              <div className="text-sm text-gray-500">Upload more cards to get started.</div>
                            </>
                          ) : selectedTab === 'ready_to_export' ? (
                            <>
                              <div className="text-sm font-medium text-gray-900">No cards ready to export yet</div>
                              <div className="text-sm text-gray-500">Review cards first to prepare them for export.</div>
                            </>
                          ) : selectedTab === 'archived' ? (
                            <>
                              <div className="text-sm font-medium text-gray-900">No archived cards</div>
                              <div className="text-sm text-gray-500">Archived cards will appear here.</div>
                            </>
                          ) : null}
                         </div>
                       </TableCell>
            ...
            <DialogHeader className="flex-shrink-0">
              <div className="flex justify-between items-center px-6 pt-6 pb-4 border-b">
                <div className="space-y-1">
                  <DialogTitle className="text-2xl font-semibold">Review Card Data</DialogTitle>
            ...
            <div className="flex-1 overflow-hidden">
              <div className="grid grid-cols-2 gap-6 h-full">
                ...
                <div className="bg-gray-50 rounded-lg p-4 overflow-y-auto">
                  <div className="space-y-4">
                    {selectedCardForReview ? reviewFieldOrder.map((fieldKey) => {
                      const fieldData = selectedCardForReview.fields?.[fieldKey];
                      const label = dataFieldsMap.get(fieldKey) || fieldKey.replace(/_/g, ' ');
                      const needsReview = fieldData?.requires_human_review === true;
                      const isReviewed = fieldData?.reviewed === true;
                      const reviewNotes = fieldData?.review_notes;
                      ...
                      if (fieldKey === 'cell') formattedValue = formatPhoneNumber(fieldData?.value);
                      if (fieldKey === 'date_of_birth') formattedValue = formatBirthday(fieldData?.value);

                      let tooltipContent = reviewNotes || (needsReview ? "Needs human review" : null);

                      return (
                          <div key={fieldKey} className="grid grid-cols-5 items-center gap-x-4 gap-y-1">
                              <Label htmlFor={fieldKey} className="text-right col-span-2 text-xs font-medium text-gray-600 flex items-center justify-end gap-1">
                            {showIcon && !isReviewed && (
                              <TooltipProvider delayDuration={100}>
                                <Tooltip>
                                  <TooltipTrigger>
                            ...
                            <TooltipProvider delayDuration={100}>
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button
                                    ...
                                    className={`h-8 w-8 p-1 ${
                                      isReviewed ? 'text-green-500' : 'text-gray-400 hover:text-gray-600'
                                    }`}
                                ...
                                <TooltipContent side="right">
                                  <p>{isReviewed ? 'Mark as needing review' : 'Mark as reviewed'}</p>
                                </TooltipContent>
                              </Tooltip>
                            </TooltipProvider>
                          )}
                    ...
                    <p className="text-gray-500 text-center mt-4">No card selected for review.</p>
                  )}
                  </div>
                </div>
              </div>
      ...
      <AlertDialog open={isArchiveConfirmOpen} onOpenChange={setIsArchiveConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to archive {selectedCardForReview ? 'this card' : `${Object.keys(rowSelection).length} ${Object.keys(rowSelection).length === 1 ? 'card' : 'cards'}`}? 
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction 
              onClick={selectedCardForReview ? confirmArchiveAction : handleArchiveSelected} 
      ...
      <AlertDialog open={isMoveConfirmOpen} onOpenChange={setIsMoveConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to move {Object.keys(rowSelection).length} {Object.keys(rowSelection).length === 1 ? 'card' : 'cards'} back to Ready to Export?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleMoveSelected} className="bg-blue-600 hover:bg-blue-700 text-white">
              Move
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      <AlertDialog open={isDeleteConfirmOpen} onOpenChange={setIsDeleteConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to permanently delete {Object.keys(rowSelection).length} {Object.keys(rowSelection).length === 1 ? 'card' : 'cards'}? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleDeleteSelected} className="bg-red-600 hover:bg-red-700 text-white">
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
